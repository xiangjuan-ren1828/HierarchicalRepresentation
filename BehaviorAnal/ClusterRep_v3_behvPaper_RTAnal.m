...
predRatio = zeros(subLen, BinL, length(expList));
errRatio  = zeros(subLen, BinL, length(expList)); %% ratio of error or time out trials
fastRatio = zeros(subLen, BinL, length(expList));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
baseAcc      = zeros(subLen, length(expList));
transTpAcc   = zeros(subLen, length(expList)); %% correct response ratio in within- and between-cluster transitions
transTpAcc_lure = zeros(subLen, 2, 2, length(expList)); % the 1st and 2nd '2' denote 'lure distractor exists vs. none' and 'transition from boundary node to within node vs. from boundary to boundary'
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
lureTrialLen        = zeros(subLen, 2 * 2, length(expList));
...
RTtransTp_type_lure = zeros(subLen, 2, 2, 2, length(expList));
RToverall_Blc_dtNum = zeros(subLen, 3, BinL, length(expList)); % 3 distractor conditions
lme_subj = [];
    ...
    for SubIdx = 1 : subLen
        ...
        for iB = 1 : BinL
            ...
            if isequal(ExpWord, 'ImplicitExp') || isequal(ExpWord, 'ImplicitRandExp')
                predRatio(SubIdx, iB, iExp) = length(find(respRT_Col(ttBlcIdx) <= grayoffT & errorIdx_Col(ttBlcIdx) ~= 1 & fast_Col(ttBlcIdx) ~= 1)) / trialsBlc; %% equaling to the ratio of 100 bonus
            elseif isequal(ExpWord, 'ExplicitExp')
                predRatio(SubIdx, iB, iExp) = length(find(bonus_Col(ttBlcIdx) == marioWin & errorIdx_Col(ttBlcIdx) ~= 1 & fast_Col(ttBlcIdx) ~= 1)) / trialsBlc;
            end
            errRatio(SubIdx, iB, iExp)  = length(find(errorIdx_Col(ttBlcIdx) == 1 | timeOIdx_Col(ttBlcIdx) == 1)) / trialsBlc;
            fastRatio(SubIdx, iB, iExp) = length(find(fast_Col(ttBlcIdx) == 1)) / trialsBlc;
        end
        ...
        baseAcc(SubIdx, iExp) = 1/2 * ((length(find(dtNum == 2))) / length(dtNum)) + ...
        ...
        transTpAcc(SubIdx, iExp) = length(find(transStyle == 1 & errorIdx_Col ~= 1 & fast_Col ~= 1)) / length(find(transStyle == 1));
        transTpAcc(SubIdx, iExp) = length(find(transStyle ~= 1 & errorIdx_Col ~= 1 & fast_Col ~= 1)) / length(find(transStyle ~= 1));
        ...
        % transition
        transTpAcc_lure(SubIdx, 1, 1, iExp) = length(find(nodesLabel == 1 & transStyle == 1 & lureIn == 1 & errorIdx_Col ~= 1 & fast_Col ~= 1)) / length(find(nodesLabel == 1 & transStyle == 1 & lureIn == 1));
        transTpAcc_lure(SubIdx, 1, 2, iExp) = length(find(nodesLabel == 1 & transStyle ~= 1 & lureIn == 1 & errorIdx_Col ~= 1 & fast_Col ~= 1)) / length(find(nodesLabel == 1 & transStyle ~= 1 & lureIn == 1));
        transTpAcc_lure(SubIdx, 2, 1, iExp) = length(find(nodesLabel == 1 & transStyle == 1 & lureIn == 0 & errorIdx_Col ~= 1 & fast_Col ~= 1)) / length(find(nodesLabel == 1 & transStyle == 1 & lureIn == 0));
        transTpAcc_lure(SubIdx, 2, 2, iExp) = length(find(nodesLabel == 1 & transStyle ~= 1 & lureIn == 0 & errorIdx_Col ~= 1 & fast_Col ~= 1)) / length(find(nodesLabel == 1 & transStyle ~= 1 & lureIn == 0));
        ...
        RTtransTp_lure(SubIdx, 1, 1, iExp) = mean(respRT_Col(find(nodesLabel == 1 & transStyle == 1 & lureIn == 1 & errorIdx_Col ~= 1 & fast_Col ~= 1)));
        RTtransTp_lure(SubIdx, 1, 2, iExp) = mean(respRT_Col(find(nodesLabel == 1 & transStyle ~= 1 & lureIn == 1 & errorIdx_Col ~= 1 & fast_Col ~= 1)));
        RTtransTp_lure(SubIdx, 2, 1, iExp) = mean(respRT_Col(find(nodesLabel == 1 & transStyle == 1 & lureIn == 0 & errorIdx_Col ~= 1 & fast_Col ~= 1)));
        RTtransTp_lure(SubIdx, 2, 2, iExp) = mean(respRT_Col(find(nodesLabel == 1 & transStyle ~= 1 & lureIn == 0 & errorIdx_Col ~= 1 & fast_Col ~= 1)));

        lureTrialLen(SubIdx, 1, iExp) = length(find(nodesLabel == 1 & transStyle == 1 & lureIn == 1 & errorIdx_Col ~= 1 & fast_Col ~= 1));
        lureTrialLen(SubIdx, 2, iExp) = length(find(nodesLabel == 1 & transStyle ~= 1 & lureIn == 1 & errorIdx_Col ~= 1 & fast_Col ~= 1));
        lureTrialLen(SubIdx, 3, iExp) = length(find(nodesLabel == 1 & transStyle == 1 & lureIn == 0 & errorIdx_Col ~= 1 & fast_Col ~= 1));
        lureTrialLen(SubIdx, 4, iExp) = length(find(nodesLabel == 1 & transStyle ~= 1 & lureIn == 0 & errorIdx_Col ~= 1 & fast_Col ~= 1));
        ...
        RTtransTp_type_lure(SubIdx, 1, 1, 1, iExp) = mean(respRT_Col(find(nodesLabel == 1 & transStyle == 1 & lureIn == 1 & errorIdx_Col ~= 1 & fast_Col ~= 1 & rndHam_Col == 1))); %% rndHam_Col == 1: random trial
        RTtransTp_type_lure(SubIdx, 1, 2, 1, iExp) = mean(respRT_Col(find(nodesLabel == 1 & transStyle ~= 1 & lureIn == 1 & errorIdx_Col ~= 1 & fast_Col ~= 1 & rndHam_Col == 1)));
        RTtransTp_type_lure(SubIdx, 1, 1, 2, iExp) = mean(respRT_Col(find(nodesLabel == 1 & transStyle == 1 & lureIn == 1 & errorIdx_Col ~= 1 & fast_Col ~= 1 & rndHam_Col == 2))); %% rndHam_Col == 2: hamiltonian trial
        RTtransTp_type_lure(SubIdx, 1, 2, 2, iExp) = mean(respRT_Col(find(nodesLabel == 1 & transStyle ~= 1 & lureIn == 1 & errorIdx_Col ~= 1 & fast_Col ~= 1 & rndHam_Col == 2)));
        % ------ no lure stimulus ------
        RTtransTp_type_lure(SubIdx, 2, 1, 1, iExp) = mean(respRT_Col(find(nodesLabel == 1 & transStyle == 1 & lureIn == 0 & errorIdx_Col ~= 1 & fast_Col ~= 1 & rndHam_Col == 1)));
        RTtransTp_type_lure(SubIdx, 2, 2, 1, iExp) = mean(respRT_Col(find(nodesLabel == 1 & transStyle ~= 1 & lureIn == 0 & errorIdx_Col ~= 1 & fast_Col ~= 1 & rndHam_Col == 1)));
        RTtransTp_type_lure(SubIdx, 2, 1, 2, iExp) = mean(respRT_Col(find(nodesLabel == 1 & transStyle == 1 & lureIn == 0 & errorIdx_Col ~= 1 & fast_Col ~= 1 & rndHam_Col == 2)));
        RTtransTp_type_lure(SubIdx, 2, 2, 2, iExp) = mean(respRT_Col(find(nodesLabel == 1 & transStyle ~= 1 & lureIn == 0 & errorIdx_Col ~= 1 & fast_Col ~= 1 & rndHam_Col == 2)));
        ...
        for iB = 1 : BinL
            ttBlcIdx = (iB - 1) * nBin + 1 : iB * nBin;
            RTtransTp_Blc(SubIdx, 1, iB, iExp) = mean(respRT_Col(find((trials_Col >= ttBlcIdx(1) & trials_Col <= ttBlcIdx(end)) & transStyle == 1 & errorIdx_Col ~= 1 & fast_Col ~= 1)));
            RTtransTp_Blc(SubIdx, 2, iB, iExp) = mean(respRT_Col(find((trials_Col >= ttBlcIdx(1) & trials_Col <= ttBlcIdx(end)) & transStyle ~= 1 & errorIdx_Col ~= 1 & fast_Col ~= 1)));
        end
        ...
        for rOh = 1 : 2 % RTtransTp_type_Blc = cell(1, 2);
            for iB = 1 : BinL
                ttBlcIdx = (iB - 1) * nBin + 1 : iB * nBin;
                RTtransTp_type_Blc{iExp, rOh}(SubIdx, 1, iB) = mean(respRT_Col(find((trials_Col >= ttBlcIdx(1) & trials_Col <= ttBlcIdx(end)) & transStyle == 1 & errorIdx_Col ~= 1 & fast_Col ~= 1 & rndHam_Col == rOh)));
                RTtransTp_type_Blc{iExp, rOh}(SubIdx, 2, iB) = mean(respRT_Col(find((trials_Col >= ttBlcIdx(1) & trials_Col <= ttBlcIdx(end)) & transStyle ~= 1 & errorIdx_Col ~= 1 & fast_Col ~= 1 & rndHam_Col == rOh)));
            end
        end
        ...
        for iDt = 1 : length(dtrNums) % RToverall_Blc_dtNum = zeros(subLen, 3, nBlock); % 3 distractor conditions
            for iB = 1 : BinL
                ttBlcIdx = (iB - 1) * nBin + 1 : iB * nBin;
                RToverall_Blc_dtNum(SubIdx, iDt, iB, iExp) = mean(respRT_Col(find((trials_Col >= ttBlcIdx(1) & trials_Col <= ttBlcIdx(end)) & dtNum == dtrNums(iDt) & errorIdx_Col ~= 1 & fast_Col ~= 1)));
            end
        end
...
%% SI figure, RT Learning effect: within- and between-transitions
% RTtransTp_Blc = zeros(subLen, 2, BinL, length(expList));
figKey = 1;
if figKey == 0
    barLineWid = 2;
...
for iExp = 1 : length(expList)
    ...
    for iTs = 1 : nTrans
        RTtrans_iWs_iTs = squeeze(RTtransTp_Blc(:, iTs, :, iExp));
        [RTsAvg, RTsSem] = Mean_and_Se(RTtrans_iWs_iTs, 1);
        colorTmp = colorSet(iTs, :);

        errorbar(1 : 1 : BinL, RTsAvg, RTsSem, 'Color', colorTmp, 'LineStyle', '-', 'LineWidth', errLineWid); hold on;
        for iB = 1 : BinL
            plot(iB, RTsAvg(iB), 'Marker', 'o', 'MarkerSize', markSize, 'MarkerEdgeColor', [0, 0, 0], 'MarkerFaceColor', colorTmp, 'LineStyle', '-'); hold on;
        end
    end
...
% RTtransTp_type_Blc    = cell(length(expList), 2); % random vs. hamiltonian walks
% RTtransTp_type_Blc{1, 1} = (subLen, transType, nBlock)
figKey = 1;
if figKey == 0
    barLineWid = 2;
    errLineWid = 3;
...
for iExp = 1 : length(expList)
    ...
    hold on;
    for i_rOh = 1 : nWalks
        RTtrans_iWs = RTtransTp_type_Blc{iExp, i_rOh};
        for iTs = 1 : nTrans
    ...
    box off;
    ax = gca;
    save_name = ['Exp', num2str(iExp), '-', expList{iExp}, '-learning-trans-walks.png'];
    %exportgraphics(ax, save_name, 'Resolution', 600);
end
...
for iExp = 1 : length(expList)
    ...
    hold on;
    for iDt = 1 : length(dtrNums) % RToverall_Blc_dtNum = zeros(subLen, 3, nBlock); % 3 distractor conditions
        RTtrans_iDt = squeeze(RToverall_Blc_dtNum(:, iDt, :, iExp));
        [RTsAvg, RTsSem] = Mean_and_Se(RTtrans_iDt, 1);
        errorbar(1 : 1 : BinL, RTsAvg, RTsSem, 'Color', blueGrad(iDt, :), 'LineStyle', LineStys{iDt}, 'LineWidth', errLineWid); hold on;
        for iB = 1 : BinL
            plot(iB, RTsAvg(iB), 'Marker', 'o', 'MarkerSize', markSize, 'MarkerEdgeColor', [0, 0, 0], 'MarkerFaceColor', blueGrad(iDt, :), 'LineStyle', '-'); hold on;
        end
    end
    xlim([0.5, BinL+0.5]);
    ylim([0.9, 1.6]);
...
for iExp = 1 : length(expList)
    ...
    for i_rOh = 1 : 2 % Random vs. Hamiltonian Walk
        ...
        %%% line plot
        plot(barPos_i, RTtransTp_type_iExp(:, :, i_rOh), 'Color', [0.6, 0.6, 0.6], 'LineStyle', '-', 'LineWidth', indvLineW); hold on;
        ...
        [h, p, ci, stats] = ttest(RTtransTp_type_iExp(:, 1, i_rOh), RTtransTp_type_iExp(:, 2, i_rOh))
    end
    xlim([0.6, 2.6]);
    ylim([0.5, 2]);
    if figKey == 0
