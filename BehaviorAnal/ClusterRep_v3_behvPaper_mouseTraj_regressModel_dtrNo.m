% ClusterRep_v3_mouseTraj_regressModel_dtrNo
% added by XR @ Feb 13 2026
% Running the regression with different distract No
% =========================================================================
...
paramsName_Prob = {'trialCnt', 'probTrans', 'probAss'}; 
paramsName_EVs  = {'trialCnt', 'EVtrans', 'EVass'}; 
nParams_Prob    = length(paramsName_Prob); % exclude intercept parameter
nParams_EVs     = length(paramsName_EVs);
subLen = 24;

% ------ Regress coefficient under differet distractor numbers ------
betas_times_dtrNone = cell(subLen, length(expList));
betas_times_dtrNtwo = cell(subLen, length(expList));
betas_times_dtrNthr = cell(subLen, length(expList));
...
for iExp = 1 : length(expList)
    ...
    for iSub = 1 : subLen
        ...
        if isequal(expMode_i, 'mouse')
            ...
            if  isequal(Midx, 'SRheurProcessTraceTruncAttmod')
            ...
            %% ------ Distractor No = 1 ------
            trainTrl_idx = find(objDtrNo_col == 1);
            regressor_train_trial = regressor_trial_raw(trainTrl_idx, :);
            regressor_train_trial(:, 2 : end) = zscore(regressor_train_trial(:, 2 : end));

            betas_iTime_one = nan(nParams_Prob, length(circle_list), 2); % 2: probability and EVs as regressors
            for iTime = 2 : length(circle_list)
                ...
                if size(logit_trial, 1) > (3 * nParams_Prob)
                    ...
                    betas_iTime_one(:, iTime, 1) = betas_Prob;
                    ...
                    betas_iTime_one(:, iTime, 2) = betas_EVs;

                end
            end
            betas_times_dtrNone{iSub, iExp} = betas_iTime_one;

            %% ------ Distractor No = 2 ------
            trainTrl_idx = find(objDtrNo_col == 2);
            regressor_train_trial = regressor_trial_raw(trainTrl_idx, :);
            regressor_train_trial(:, 2 : end) = zscore(regressor_train_trial(:, 2 : end));

            betas_iTime_two = nan(nParams_Prob, length(circle_list), 2); % 2: probability and EVs as regressors
            for iTime = 2 : length(circle_list)
                choiceId_i  = choiceId_time(:, iTime);
                choiceId_i  = choiceId_i(trainTrl_idx);
                % ------ Constructing the GLM array ------
                ...
                if size(logit_trial, 1) > (3 * nParams_Prob)
                    ...
                    betas_iTime_two(:, iTime, 1) = betas_Prob;
                    ...
                    betas_iTime_two(:, iTime, 2) = betas_EVs;

                end
            end
            betas_times_dtrNtwo{iSub, iExp} = betas_iTime_two;


            %% ------ Distractor No = 3 ------
            trainTrl_idx = find(objDtrNo_col == 3);
            regressor_train_trial = regressor_trial_raw(trainTrl_idx, :);
            regressor_train_trial(:, 2 : end) = zscore(regressor_train_trial(:, 2 : end));

            betas_iTime_three = nan(nParams_Prob, length(circle_list), 2); % 2: probability and EVs as regressors
            for iTime = 2 : length(circle_list)
                choiceId_i  = choiceId_time(:, iTime);
                choiceId_i  = choiceId_i(trainTrl_idx);
                % ------ Constructing the GLM array ------
                ...
                if size(logit_trial, 1) > (3 * nParams_Prob)
                    ...
                    betas_iTime_three(:, iTime, 1) = betas_Prob;
                    ...
                    betas_iTime_three(:, iTime, 2) = betas_EVs;

                end
            end
            betas_times_dtrNthr{iSub, iExp} = betas_iTime_three;
...
save([dataSave_folder, 'betas_regress_times_dtrNo.mat'], 'betas_times_dtrNone', 'betas_times_dtrNtwo', 'betas_times_dtrNthr');
...
trialFlg = 3; % 1-dtrNo=1; 2-dtrNo=2; 3-dtrNo=3;
if trialFlg == 1
    betas_times_tmp = betas_times_dtrNone;
elseif trialFlg == 2
    betas_times_tmp = betas_times_dtrNtwo;
elseif trialFlg == 3
    betas_times_tmp = betas_times_dtrNthr;
end

dataFlg  = 1; % 1-predicted choice probability as the regressor; 2-EVs
betas_times_plot = nan(2, length(circle_list), subLen, length(expList)); % 2: betas from memory and prediction system 
% paramsName_Prob = {'trialCnt', 'probTrans', 'probAss'}; 
% paramsName_EVs  = {'trialCnt', 'EVtrans', 'EVass'}; 
for iExp = 1 : length(expList)
    for iSub = 1 : subLen
