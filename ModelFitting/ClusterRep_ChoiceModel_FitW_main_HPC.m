function ClusterRep_ChoiceModel_FitW_main_HPC(iExp, iMode, iSub)
% modified by XR @ Feb 14 2026 based on
% ClusterRep_v3_behvPaper_mouseTraj_WeightFit.m.
% =================================================================
% -------- modified by XR @ Jan 13 2026 based on --------
% ClusterRep_ChoiceModel_main
% Run exactly one experiment + one participant as one HPC job.
% Usage:
%   matlab -batch "ClusterRep_ChoiceModel_main_HPC(1,1,5)"
%   iExp  : 1..length(expList)
%   iMode : 1..length(expModeList)  (1=mouse, 2=key)
%   iSub  : 1..length(subj_list) for that experiment

if nargin < 1 || isempty(iExp);  iExp  = 1; end
if nargin < 2 || isempty(iMode); iMode = 1; end
if nargin < 3 || isempty(iSub);  iSub  = 1; end

clearvars -except iExp iMode iSub
clc
...
Midx_clamp  = 'choiceW_refit';
refit_clamp = 1; % refit the weight for the SR component
...
if iSub < 1 || iSub > subLen
    error('iSub out of range for %s. Must be 1..%d', ExpWord, subLen);
end
...
fprintf('=== Running ExpWord = %s (iExp=%d), expMode=%s (iMode=%d), subject=%s (iSub=%d/%d) ===\n', ...
        ExpWord, iExp, expMode_i, iMode, subjBv, iSub, subLen);
...
if isequal(expMode_i, 'mouse')
    ...
    % ------ Only one model ------
    Midx      = 'SRheurProcessTraceTruncAttmod';
    priorWord = 'nullPrior';
    ...
    %% ------ All trials together ------
    % Refit the weight parameter for SR component by assuming
    % multinomial noise
    % ------ Loop over the time point and refit the weight parameter ------
    for iTime = 2 : length(circle_list)
        choiceId_i = choiceId_time(:, iTime);
        if sum(~isnan(choiceId_i)) > 5
            [lliEst_clamp, paramsEst_clamp] = ClusterRep_clampOthers_FitW(ExpWord, subID, stim, resp, choiceId_i, fitWord, ...
                posXY, respYes_trials_blc, mouseTraj_trials_blc, objAng_col, objDtrNo_col, miniDmat, nFit, Midx, Midx_clamp, folder, refit_clamp, p_choice_trs, p_choice_ass, iTime);
        end
    end 
